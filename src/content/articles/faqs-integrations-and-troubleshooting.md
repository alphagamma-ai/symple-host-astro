...
SympleHost connects to major OTAs including **Airbnb**, **Booking.com**, **Expedia**, **Agoda**, **VRBO**, **MakeMyTrip**, and **Trip.com**, depending on account access. Open **Channel Manager** for the full list of available connections.
...
Open **Channel Manager**, find the OTA connection, and click **Disconnect**. For OTA channels such as Airbnb and Booking.com, also check the OTA-side disconnect steps below.

### How do I disconnect Airbnb?
Airbnb connects directly to SympleHost via OAuth, so disconnecting is a two-side process:

1. **In SympleHost:** Open **Channel Manager**, find the **Airbnb** connection, and click **Disconnect**.
...
Booking.com syncs through **Channex.io** (the connectivity provider). That means there are two places where the connection lives, and **you must remove it from both**, otherwise Booking.com will keep treating Channex.io as your active channel manager and your rates/availability will get stuck:

1. **In SympleHost:** Open **Channel Manager**, find the **Booking.com** connection, and click **Disconnect**. This stops SympleHost ↔ Channex.io sync.
...
2. **In the Booking.com Extranet (do not skip this):**
   ...
   - Find **Channex.io** in the list and click **Disconnect**.
...
⚠️ **Until you complete step 2**, Booking.com keeps allowing Channex.io to control your rates and availability — meaning rates may not update, and you may not be able to manually edit them in the extranet either. This is a Booking.com behaviour, not a SympleHost one.
